...
CMatrix* CBillBoard::spModelViewInverse = nullptr;

void CBillBoard::ModelViewInverse(CMatrix* matrix)
{
	spModelViewInverse = matrix;
}
...
void CBillBoard::Render(CMaterial *mpMaterial) {
	...
	glPushMatrix();
	//拡縮、回転、移動させる
	glMultMatrixf((*spModelViewInverse * mMatrix).M());
	//ライトオフ
