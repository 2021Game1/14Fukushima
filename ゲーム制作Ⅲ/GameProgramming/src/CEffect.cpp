...
CMaterial CEffectEnemyDamageSp3::sMaterial; //マテリアル.テクスチャ
...
CEffectEnemyDamageSp3::CEffectEnemyDamageSp3(const CVector& pos, float w, float h, char* texture, int row, int col, int fps)
	: CBillBoard(pos, w, h), mRows(row), mCols(col), mFps(fps), mFrame(0)
{
	//テクスチャを読んでない場合は読む
	if (sMaterial.Texture()->Id() == 0)
...
void CEffectEnemyDamageSp3::Update() {
	//コマ数の計算
	int frame = mFrame++ / mFps;
	if (frame == mRows * mCols)
	{
...
void CEffectEnemyDamageSp3::Render()
{
	glDisable(GL_DEPTH_TEST); //深度テスト無効
	CBillBoard::Render(&sMaterial);
	glEnable(GL_DEPTH_TEST); //深度テスト有効
