	...
	, CVector* adjust)
{
	...
	//最短のベクトルを求める
	*adjust = VectorLineMinDist(mV0 + r1, mV1 - r1, oV0 + r2
		, oV1 - r2);
	if (adjust->Length() == 0.0f)
		return true;
	if (adjust->Length() < m->mRadius + o->mRadius)
	{
		*adjust = adjust->Normalize() * (m->mRadius
			+ o->mRadius) - *adjust;
		return true;
	}
	return false;
...
CVector CCollider::VectorLineMinDist(const CVector& Start1
	, const CVector& End1, const CVector& Start2, const CVector& End2)
{
